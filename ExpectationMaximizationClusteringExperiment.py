...
class ExpectationMaximizationClusteringExperiment(object):
    ...
    def cluster_and_graph_reduced_data(self, reduced_data, reduction_method="idk"):
        ...
        for i in range(len(reduced_data[0])):
            ...
            x_axis_margin = abs(max_val - min_val) * 0.01
            plt.xlim([min_val - x_axis_margin, max_val + x_axis_margin])
            plt.title("em with k=" + str(k) + " for file: " + self.data_set_file_name + ", " + reduction_method + " ftr. " + str(i + 1))
            plt.show()
        ...
        plt.title("em, k=" + str(k) + ", " + reduction_method + " reduction, file: " + self.data_set_file_name)
        plt.show()

        print("Expectation Maximization Clustering Experiment with " + reduction_method + " has been run for file " + self.data_set_file_name + ".csv")